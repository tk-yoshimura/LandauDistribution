...
namespace PDF {
    class ScaledPDF {
        static void Main() {
            ...
            for (MultiPrecision<N12> h = 1d / 512, x0 = 1, x1 = 2; x1 <= 8192; h *= 2, x0 *= 2, x1 *= 2) {
            ...
            for (MultiPrecision<N12> h = 1d / 512, x0 = 1, x1 = 2; x1 <= 16; h *= 2, x0 *= 2, x1 *= 2) {
                for (MultiPrecision<N12> x = x0; x < x1; x += h) {
                    xs.Add(-x);
                }
            }
            ...
            using StreamWriter sw = new("../../../../results_disused/integrate_scaled_pdf_precision80.csv");
            ...
            foreach (MultiPrecision<N12> x in xs) {
                ...
                if (x.Sign == Sign.Plus) {
                    (MultiPrecision<N12> y, MultiPrecision<N12> err) = NumericIntegration.PDFPositiveSide<N12>.Value(x, 1e-80);
                    ...
                    sw.WriteLine($"{x},{y:e100},{err:e8}");
                    sw.Flush();
                }
                else {
                    (MultiPrecision<N12> y, MultiPrecision<N12> err) = NumericIntegration.PDFNegativeSide<N12>.ScaledValue(x, 1e-80);

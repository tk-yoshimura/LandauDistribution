...
namespace PadeApproximate {
    class QuantileForward {
        static void Main_() {
            ...
            List<(MultiPrecision<N16> cdf, MultiPrecision<N16> lambda)> expecteds = ReadExpacted();
            ...
            List<(MultiPrecision<N16> c0, MultiPrecision<N16> c1, bool log2scale)> ranges = new() {
                ...
                (MultiPrecision<N16>.Ldexp(1, -4), MultiPrecision<N16>.Ldexp(1, -16), true),
                (MultiPrecision<N16>.Ldexp(1, -16), MultiPrecision<N16>.Ldexp(1, -64), true),
                (MultiPrecision<N16>.Ldexp(1, -64), MultiPrecision<N16>.Ldexp(1, -256), true),
                (MultiPrecision<N16>.Ldexp(1, -256), MultiPrecision<N16>.Ldexp(1, -1024), true),
                (MultiPrecision<N16>.Ldexp(1, -1024), MultiPrecision<N16>.Ldexp(1, -4096), true),
                (MultiPrecision<N16>.Ldexp(1, -4096), MultiPrecision<N16>.Ldexp(1, -16384), true),
            };
            ...
            foreach ((MultiPrecision<N16> u0, MultiPrecision<N16> u1, bool log2scale) in ranges) {
                ...
                for (int k = 20; k <= 64 && k * 2 + 1 < xs.Length; k++) {
                    ...
                    if (success) {
                        using StreamWriter sw = new($"../../../../results_disused/padecoef_cdf_precision81_{s0}_{s1}_{(log2scale ? "log2": "linear")}.csv");
        ...
        private static List<(MultiPrecision<N16> lambda, MultiPrecision<N16> scaled_cdf)> ReadExpacted() {
            ...
            List<(MultiPrecision<N16> cdf, MultiPrecision<N16> lambda)> expecteds = new();
            StreamReader stream = new("../../../../results_disused/quantile_cdf_precision82_3.csv");
            stream.ReadLine();
            ...
            while (!stream.EndOfStream) {
                ...
                MultiPrecision<N16> cdf = MultiPrecision<N16>.Ldexp(item[1], int.Parse(item[0].Split('^')[0]));
                MultiPrecision<N16> lambda = item[2];

namespace LandauNegativeSideRoot {
    public static class LandauNegativeSideInverse {
        ...
        private static double SearchRootDecreasePhase(double x, double v, double t0, double t_peak) {
            ...
            for (int i = 0; i < 256; i++) {
                ...
                if (double.IsFinite(dt_prev)) {
                    if (Math.Abs(dt) > Math.Abs(dt_prev) || dt * dt_prev < 0) {
                        dt /= 4;
                    }
                }
        ...
        public static IEnumerable<double> EnumHalfPIValues(double x, int maxpoints = 32) {
            ...
            for (i = 1, j = -1; i <= decrease_points; i++, j--) {

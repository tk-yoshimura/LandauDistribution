...
namespace Digits82 {
    class CDFIntegrate {
        static void Main_() {
            ...
            {
                using StreamWriter sw = new("../../../../results_disused/asymptotic_pade_pdfintegrate_precision82.csv");
                sw.WriteLine("lambda0,lambda1,integrate,error");
                ...
                foreach (MultiPrecision<N12> x1 in xs.Skip(1)) {
                    ...
                    if ((x0 >= -2 && x0 <= 2) & (x1 >= -2 && x1 <= 2)) {
                        eps = (PDF.Value(x0) + PDF.Value(x1)) * (x1 - x0) * 1e-83;
                    }
                    ...
                    (MultiPrecision<N12> y, MultiPrecision<N12> error) = GaussKronrodIntegral<N12>.AdaptiveIntegrate(PDF.Value, x0, x1, eps, GaussKronrodOrder.G32K65, 128);

                    Console.WriteLine($"{x0},{x1},{y:e16},{eps:e8},{error:e8}");
                    sw.WriteLine($"{x0},{x1},{y},{error:e8}");
                    sw.Flush();
